...
export const drawRuller = (measures) => {
    Object.entries(measures).forEach(([direction, style]) => {
        const ruller = $(
            `<div class="rb-zeplin-ruller rb-zeplin-ruller-${direction}"></div>`,
        );
        ruller.css(style);
        $('body').append(ruller);
    });
...
export const drawDistanceLine = ({
    ...
    if (heightOfVerticalGuide < 15) {
        ...
        veriticalLine.append(
            `<span class="rb-zeplin-guide-distance rb-zeplin-guide-distance-vertical">${Math.abs(
                parseInt(heightOfVerticalGuide),
            ) - 1}px</span>`,
        );
    }
    if (widthOfHorizontalGuide < 15) {
        ...
        horizontalLine.append(
            `<span class="rb-zeplin-guide-distance rb-zeplin-guide-distance-horizontal">${Math.abs(
                parseInt(widthOfHorizontalGuide),
            ) - 1}px</span>`,
        );
    }
